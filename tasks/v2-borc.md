...
| **V2-107** | **32 ekran cümlesi sayıyı ELLE taşıyor — bugün hepsi doğru, hiçbiri türetilmemiş** | Kapanış taraması: 48 ekran dosyasındaki JS dize sabitleri içinde ölçüm cümlesi kalıbında sayı taşıyan **36 cümle** bulundu. Her biri veriye karşı tek tek sınandı (`DB.tickets` 7 · `slaPolicies` 7/7 · `projects` 14/14 · `invoices` 17/17 · `tasks` 26 · `leaves` 7 · `employees` 16 · `purchases` 2/7 · `assignments` 7/7 · `policies` 14 alan · `company.aylikCalismaSaati` 176 = 22×8): **36'sının 36'sı bugün DOĞRU, sıfır bayat**. Bu turda **4'ü türetilmiş hâle getirildi** (`app-destek-form.html`de üç istatistik cümlesi ve gerçek bir talep başlığından alınmış bir `placeholder`) çünkü portal kimliğinde KAPSAMSIZ sayı basıyorlardı — yani aynı anda V2-100 sınıfı sızıntıydılar. Kalan **32'si doğru ama türetilmemiş**: defter bir kayıt büyüdüğünde sessizce yalana dönerler. Bunları ölçen bir eksen YOK — `brief-dogrula.js` `[B5]` brief'in iddialarını ölçer, EKRAN cümlelerini değil |
