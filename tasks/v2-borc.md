...
## Dilim 7 (gömme) — ajan açılmadan ÖNCE ölçülerek doğan borç

> ⚠️ Bu iki madde **aynı sınıftandır** ve dilim 6'nın GRUP 1 blokajının
> genelleştirilmiş hâlidir. Dilim 6'da blokaj ajan AÇILDIKTAN sonra
> bulunmuştu; bu turda ölçüm ajandan önce yapıldı ve **12 ajan boşa
> açılmadı**. Ölçüm yöntemi: bir deftere kayıt EKLEYEN yordam ortak
> katmanda `push`/`unshift`/`splice` ile görünür. İki yönlü sınandı —
> yordamı olduğu bilinen altı defterde ölçüm pozitif döndü (`DB.tasks` 2 ·
> `DB.tickets` 1 · `DB.quotes` 1 · `DB.personalNotes` 3 · `DB.contracts` 1 ·
> `DB.projects` 1).

| # | Madde | Ölçüm |
|---|---|---|
| **V2-98** | **Araç alt defteri yazma yordamı YOK** (rota 102·104·106·108·110·112) | Altı alt defter — `DB.maintenance` · `DB.inspections` · `DB.policies` · `DB.fuelLogs` · `DB.vehicleExpenses` · `DB.accidents` — hiçbirine kayıt ekleyen yordam yok. **Beyar kararı (13 Ağustos): yazma yordamı YAZILMAYACAK, kapsam donduruldu.** Drawer'lar SALT OKUNUR açılır; kaydetme düğmesi **gizlenmez, devre dışı bırakılır** ve sebebi ekranda yazılır. Rota satırları KAPATILMADI. V2 borcu olarak **"araç alt defteri yazma yordamı"** adıyla burada durur |
| **V2-99** | **On iki gömme satırının yazma yordamı YOK** (rota 23·29·35·38·41·44·47·53·64·85·119·122) | Ölçülen defterler ve ekleyen yordam durumu: `DB.analyses` YOK · `DB.commissions` YOK · `DB.sprints` YOK · `DB.testCases` YOK · `DB.bugs` YOK · `DB.changeRequests` YOK · `DB.deliveries` YOK · `DB.deptRequests` YOK · `DB.meetings` YOK · `DB.performance` YOK · `DB.orders` YOK · `DB.suppliers` YOK. On iki drawer satırı için **ajan açılmadı**; kaydetmesi olmayan form basmak §14.6'nın form ölçeğidir. ⚠️ **BEYAR KARARI GEREKİYOR:** V2-98'in kararı (salt okunur drawer + devre dışı düğme + sebep) bu on iki satıra da uzatılsın mı, yoksa bu satırlar yüzeysiz mi kalsın? Uzatmak sekiz ayrı ekrana dokunmak demektir — bu turda kapsam DONDURULDU ve karara bırakıldı |

## Dilim 7 — panel turunda doğan borç

| # | Madde | Ölçüm |
|---|---|---|
| **V2-100** ✅ **KAPANDI** (ADR-R2-47) | ~~**Müşteri portalı kimliği panelde İÇ VERİ görüyor**~~ | Bu turun eklemesi DEĞİL, önceden de böyleydi; rota 2·5 yazılırken ölçüldü (`YTK-001` oturumu, `emp:null`): "Son bildirimler" **5 iç bildirim** (`BLD-*`, `kisi:EMP-*`) · "Proje özeti" **6 iç proje** · "Ekip özeti" **15 personel** ve izin talepleri **isimleriyle** · KPI "Bekleyen onay = 10" iç onayları sayıyor. Bu turda eklenen üç çekmece (Dikkat · Dünden bugüne · Duyurular) bu kimliğe **kapalı** — `GV.session.emp` ve `GV.perm.sec('ekip')` kapılarından geçiyorlar. Var olan dört kart **açık**. Var olan davranışı bozmama yükümlülüğü gereği dokunulmadı. **Kişisel veri kapısının (§20.1) panel tarafındaki karşılığı yazılmamış.** · ✅ **13 Ağustos kapanışı:** yayılım ölçüldü ve panelden ÇOK DAHA GENİŞ çıktı — müşterinin ulaşabildiği **14 yüzeyin 5'i** sızdırıyordu (**50 kişi adı · 130 iç/yabancı kayıt**), `app-gorev-form.html` ve `app-proje-form.html` YENİ KAYIT kipinde bütün kadroyu ve bütün projeleri `<option>` olarak basıyordu. Kimlik düzeyi kapı ortak katmana yazıldı (`GV.perm.icVeri` · `icVeriNeden` · `icKume` · `kapsa` · `GV.guardIcEkran`), `GV.guardRecord` yabancı kaydın ADINI basmayı bıraktı. Onarım sonrası: **0 sızan yüzey · 0 isim · 0 kayıt**. Yeni eksen `tasks/qa/musteri-kapisi.js`, iki yönlü sınandı. Ayrıntı: **ADR-R2-47** |
| **V2-101** | **Ortak katmanda beş boşluk — panel turunda ölçüldü, hiçbirine yazılmadı** | (1) `GV.fmt`te GÜN EKLEYEN yordam yok (`days` var, `addDays` yok) — "dün"ü her ekran kendi türetiyor; (2) VADE/YENİLEME EŞİĞİ yordamı yok — `GV.yenileme` bambaşka iş yapıyor (`TUR_ALAN` yalnız `sozlesme`+`paket`, işi bir yıl UZATMAK), poliçe/muayene/bakım defterlerinde eşik alanı da yok; 60 günlük eşik iki ekranda elle ve **birebir aynı** yazılı; (3) `GV.list`in satır kapsamı süzgeci (`afterScope`) bileşen İÇİNDE hapis — dışarıdan çağrılabilir bir yordam yok, panel `GV.perm.scope('gor')` dallanmasını elle tekrarladı (L-40); (4) araç kodu → plaka çözücü yok, en az iki ekran kendi kopyasını yazıyor; (5) NPS eşiklerini (`9-10/7-8/0-6`) hesaplayan yordam yok — `ui.js:278` yalnız ton sözlüğü, eşikler şu an `app-rapor.html`de yaşıyor |
| **V2-102** | **Denetim izinde toplu veri göçü HAREKET gibi görünüyor** | `2026-08-03T10:00`da `EMP-001` tarafından yazılmış **8 adet** "Proje kaynağı belirlendi" satırı, panelin "Dünden bugüne" penceresinin en yeni 8 satırının neredeyse tamamını dolduruyor. Kart dürüst davranıp defterde ne varsa basıyor; içerik süzmek uydurma kural olurdu. Veri tarafında ayıklanması gereken gürültü |
| **V2-103** | **`DB.announcements` R1'in iki alanını taşımıyor** | `bitis` (duyurunun süresi doldu ekseni) ve `ekler` (ek dosya) alanları R2 defterinde YOK. Duyuru çekmecesi yalnız `aktif` eksenini okuyor; "süresi dolmuş duyuru" sorusu R2'de sorulamıyor ve ek dosya yüzeyi üretilemiyor. Uydurulmadı |

## Dilim 7 kapanışı — V2-100 turunda doğan borç

| # | Madde | Ölçüm |
|---|---|---|
| **V2-104** | **`app-destek-form.html` "İlgili proje" seçeneği YANLIŞ YORDAMLA etiketleniyor** | Seçenek etiketi `GV.destek.paketAdi(p) + ' · ' + hesapAd(p.musteri)` ile kuruluyor. `paketAdi` bir BAKIM PAKETİNİ adlandırır (`[p.seviye, p.tip].filter(Boolean).join(' ')`); bir PROJE kaydında ne `seviye` ne `tip` vardır, dolayısıyla yordam `null` döner. Ölçüldü (gerçek Chromium): 14 seçeneğin 14'ü `null · <hesap adı>` basıyor — alan seçilebiliyor ama seçilen projenin ADI hiçbir yerde görünmüyor. Doğru okuma `p.ad`dır. ⚠️ **Bu turda KAPATILMADI:** kapanış turunda kapsam donduruldu ve bu bir sızıntı değil, bir etiket kusurudur. Kusur V2-100 onarımı sırasında ölçüldü, uydurulmadı |
| **V2-105** ⚠️ **KARAR BEKLİYOR** | **Müşteri KENDİ kaydında iç aktörlerin adını görüyor** | Ölçüldü: `app-destek-detay.html?id=DST-2026-120` (`musteri:MUS-2024-001` = oturumun kendi hesabı) müşteri kimliğinde talebi üstlenen uzmanın adını (`Ayşe Kaplan`) 1 künye satırında ve denetim izinin 2 satırında basıyor. Bu **başka bir firmanın verisi değil**, **toplayan bir iç defter de değil** — müşterinin kendi kaydının taraflarıdır ve portal ürünlerinde genelde İSTENEN davranıştır. Karar verilmediği için **ne kapatıldı ne susturuldu**: `tasks/qa/musteri-kapisi.js` `KARARA_AÇIK` listesinde sebebiyle yazılı ve her koşumda basılıyor. Karar: müşteri kendi kaydının iç aktörlerini görsün mü, yoksa yalnız "Destek ekibi" gibi bir kurum adı mı görsün? |
| **V2-106** | **Panelin ve destek formunun satır kapsamında `kendi`/`departman` eşlemesi YOK** | `GV.perm.kapsa` bu turda ortak katmana yazıldı ve panel kartları ile destek formunun seçenek listeleri onu çağırıyor — ama **yalnız `musteri` ekseni** bildiriliyor. Sebep ölçüldü: `kendi` kapsamının PROJE tarafındaki doğru eşlemesi tek alan değil bir YORDAMDIR (`app-proje.html`: `pm ∪ teknikSorumlu ∪ musteriSorumlu ∪ ekip`); ikinci ve daha DAR bir kopyasını panele yazmak aynı soruya iki cevap üretirdi (L-40). Sonuç: `gor:'kendi'` ya da `'departman'` kapsamlı bir personel panelde HÂLÂ tüm projeleri sayıyor. **Bu turda bilerek kapatılmadı** (kapsam donduruldu · personel tarafı 10 rolde önce/sonra karşılaştırıldı, çıktı birebir aynı) ama SESSİZ de geçmiyor: eşlenmemiş kapsamın notu `GV.perm.kapsa`dan gelip özet şeridinin altında basılıyor. Kapatmanın doğru yolu eşlemeyi kopyalamak değil, `app-proje.html`in yordamını ortak katmana taşımaktır |
| **V2-107** | **36 ekran cümlesi sayıyı ELLE taşıyor — bugün hepsi doğru, hiçbiri türetilmemiş** | Kapanış taraması (`tasks/qa/beyan-tarama.js` — **kapı değil, tarama**; çıkış kodu bilerek 0, çünkü bir sayının bugün doğru olması türetilmiş olduğu anlamına gelmez). 48 ekran dosyasındaki JS dize sabitleri (yorumlar HARİÇ — yorum ekranda basılmaz) tarandı: onarım öncesi ağaçta **41**, bugün **36** cümle. Her biri veriye karşı tek tek sınandı (`DB.tickets` 7 · `slaPolicies` 7/7 · `projects` 14/14 · `invoices` 17/17 · `tasks` 26 · `leaves` 7 · `employees` 16 · `purchases` 2/7 · `assignments` 7/7 · `policies` 14 alan · `company.aylikCalismaSaati` 176 = 22×8): **36'sının 36'sı bugün DOĞRU, sıfır bayat**. Bu turda **4 cümle türetilmiş hâle getirildi** (biri iki kaynak satırına yayıldığı için tarama 5 parça düşürdü) (`app-destek-form.html`de üç istatistik cümlesi ve gerçek bir talep başlığından alınmış bir `placeholder`) çünkü portal kimliğinde KAPSAMSIZ sayı basıyorlardı — yani aynı anda V2-100 sınıfı sızıntıydılar. Kalan **36'sı doğru ama türetilmemiş**: defter bir kayıt büyüdüğünde sessizce yalana dönerler. Bunları ölçen bir eksen YOK — `brief-dogrula.js` `[B5]` brief'in iddialarını ölçer, EKRAN cümlelerini değil |
| **V2-108** | **Ceza defteri çekmecesi ROTA DIŞI EKLENTİDİR** | `app-arac-detay.html` ceza listesi salt okunur çekmece aldı (12 Ağustos, tutarlılık gerekçesiyle: altı araç alt defterinden beşi çekmece alırken birinin almaması tutarsız yüzey bırakırdı). **Beyar 13 Ağustos'ta ONAYLADI, kalıyor.** Ama rota defterinde bu yüzeyin satırı YOKTUR — rota 111 Kaza ve Cezayı birlikte sayar, ceza formunun ayrı satırı hiç açılmamıştır. Yüzey bu yüzden hiçbir rota satırını kapatmaz ve kapanış sayımına girmez; burada **rota dışı eklenti** olarak kayıtlıdır ki envanterle defter ayrışmasın |
| **V2-109** | **Rota 28 — `GV.rapor` satır derinleşme yordamı hâlâ YOK** | Rota 28 (komisyon dökümü) hücre içi `<details>` ile kapatıldı; **Beyar 13 Ağustos'ta bu biçimi ONAYLADI** ve ortak katmana derinleşme yordamı yazılmamasına karar verdi. Borç olarak burada durur: `GV.rapor` bir rapor satırına tıklandığında alt kırılımı açan bir yuva TAŞIMIYOR (`rapor.js` tarandı). İkinci bir rapor aynı ihtiyacı duyduğunda `<details>` kopyalanacak — L-40'ın tohumu. Yordam yazılırsa hücre içi `<details>` onun ilk çağıranı olur |
